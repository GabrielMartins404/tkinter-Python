...
def cancelar():
    app = Tk()
    app.title("Salvar alteração")
    app.configure(background = "#ddd")
    app.geometry("400x300")

    title = Label(app, text="Salvar Edição", background = "#ddd", foreground = "black", font = ("arial", 16, "bold"), anchor = W )
    title.grid(row=0, column=0, padx=60, pady=20)

    msg = Label(app, text="Deseja realmente salvar esta edição?.", background="#ddd", foreground="black", font=("arial", 12, ), anchor=W)
